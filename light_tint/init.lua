...
local tint_alpha = setting_int("light_tint_alpha", 82)
...
local player_overlay_max_alpha = setting_int("light_tint_player_overlay_max_alpha", 50)
...
minetest.register_on_mods_loaded(function()
	...
	for base_name, base_def in pairs(minetest.registered_nodes) do
		...
		if is_forced_moreores_node(base_name, base_def) then
...
local function update_player_overlay(player, color_name, strength)
	...
	local alpha = clamp(math.floor(strength * player_overlay_max_alpha + 0.5), 0, player_overlay_max_alpha)
	if alpha <= 0 then
		clear_player_overlay(player)
		return
	end

	local text = "blank.png^[colorize:" .. color_hex .. ":" .. alpha
	local name = player:get_player_name()
	local rec = player_overlay_hud[name]
	if rec and rec.id then
